...
class MeizituSpider(scrapy.spiders.Spider):
    ...
    start_urls=['http://www.meizitu.com/a/cute.html',]#起始链接列表
    for i in range(1,5):
        start_urls.append('http://www.meizitu.com/a/cute_'+str(i)+'.html')
    ...
    def parseImageArticle(self,response):
        ...
        src_links=response.xpath('//img[re:test(@src,"http://mm.chinasareview.com/wp-content/uploads/20\d{2}a/\d{2}/\d{2}/\d+.jpg")]/@src').extract()
        print src_links
        #获取图片集名称，用以创建文件夹
        ...
        #获取当前页面的其他图片集链接
        links = response.xpath('//a[re:test(@href,"http://www.meizitu.com/a/\d{4}.html")]/@href').extract()
        print links
        #请求其他图片集，使用此方法解析
        for url in links:
